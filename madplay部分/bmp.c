...
#define left 3
#define right 4
char *resources_path = "/home/project/mp4player/resources";
char *return_page = "/home/project/mp4player";
//建立链表
Head *creat_pic()
{
...
void insert_pic(Head *p,char *pic_path)
{
    ...
    //字符串最后一个字母的位置
    //printf("%s %c\n",insert_pic->pic_path,*(insert_pic->pic_path+strlen(pic_path)-1));
    if(!p->first)
    {
        p->first = insert_pic;
        p->last = insert_pic;
...
{
    DIR *pic_dir = opendir(resources_path); //打开一个目录
    if(!pic_dir)
    {
    ...
    while(dirp = readdir(pic_dir))
    {
        ...
        sprintf(filename,"%s/%s",resources_path,dirp->d_name);
        //printf("%s\n",dirp->d_name);
